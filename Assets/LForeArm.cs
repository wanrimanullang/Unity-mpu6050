...
public class LForeArm : MonoBehaviour {
	string getcom;
	...
	void Update () {
		...
		if (number == 4) {
			...
			Quaternion rotation_x = Quaternion.AngleAxis (angle_z*(-1)-30, Vector3.right);
			Quaternion rotation_y = Quaternion.AngleAxis (angle_x*(-1), Vector3.up);
			Quaternion rotation_z = Quaternion.AngleAxis (angle_y*(-1), new Vector3 (0,0,1));
